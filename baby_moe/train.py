...
mode = "gpt"  # mode to run with
num_experts = 12  # number of experts to run with
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
print(
    "loading = ",
    os.path.join(get_root_py_fpath(), "nano_gpt", "configurator.py"),
)
exec(
    open(
        os.path.join(get_root_py_fpath(), "nano_gpt", "configurator.py")
...
if init_from == "scratch":
    ...
    model = GPT(gptconf) if mode == "gpt" else MoEGPT(gptconf, num_experts)
